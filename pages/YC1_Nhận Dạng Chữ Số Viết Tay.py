...
def main():
    ...
    st.markdown('<style>h1{color: #66FF99;}</style>', unsafe_allow_html=True)
    ...
    if btn_tao_anh:
        ...
        st.image(image, width=405)
        result = recognize_digits(st.session_state.index, st.session_state.image_path)
        st.write('<span style="font-family:Consolas; color:white; font-size: 35px;">Nhận dạng:</span>',unsafe_allow_html=True)
